...
<% 
	...
	String g2bCode=comUtil.retSpace(request.getParameter("g2bCode"));
    String recept1      = "";                                                // 인증서접수번호
    String g2bCodeGubun = ""; 
    String XXXXXX       = "XXX";
    
    // 사용_접수수요기관마스터 테이블 정보 
    UM_RAE_GovuA010b ettcode=ctl1.select_gmaster(g2bCode);
...
//     String masterCode=ettcode.getg2bCode();          // 수요기관코드 
    ...
    ett2     = ctl1.selectMath(g2bCode);
    String msg   = "";
    String toUrl = "";
    
    if (g2bCode.substring(0, 2).equals("Z0")) g2bCodeGubun = "B";
    
        UM_ADJ_GovuA020b ettLicense = null; 

        UM_ADE_GovuA040b[] ett=ctl1.select_gmath(g2bCode); 
            
        boolean isMakedCode=false; 

        // 기관코드가 Z 로 시작하는 것은 신규등록시  
        // 프로그램에서 임의로 만들어낸 코드이다. 
        if( g2bCode.substring(0,2).equals("ZD")  ||   g2bCode.substring(0,2).equals("ZI") || 
        		g2bCode.substring(0,2).equals("ZP")  ||   g2bCode.substring(0,2).equals("ZR") || 
        		g2bCode.substring(0,2).equals("ZC")  ||   g2bCode.substring(0,2).equals("Z0")){ 
            isMakedCode=true; 
        }
		...
		<table width="100%" border="0" cellspacing="1" cellpadding="2"
			...
			<tr> 
				...
				<td class="tdb"  width="30%" colspan="3">
					<input type="text" name="mathCode" value="<%=(ett!=null && ett.length >0)?ett[0].getmathCode():"" %>" class="read" size="20" maxlength="13" style="width: 150px" >
            	</td>
			</tr>
		...
		<br> 
				...
				<table width="100%" border="0" cellspacing="1" cellpadding="2" class="tr">
					...
					<tr>
						...
						<td class="tdb"  colspan="3"> <input  type="radio" checked="true" name="choiceCA" class="read" value="5"> Cục Quản lý Đấu thầu - Bộ Kế hoạch và Đầu tư </td>
					</tr>
					<tr>		 
			            <td height="2" colspan="4" class="line"></td> 
	...
	function js_submit() { 
        ...
        */
       	if (confirm("Lưu thông tin cơ bản của bên mời thầu?")) { 
       		document.forms[0].action="/servlet/servlets/UM_ADJ_GovrB020y";

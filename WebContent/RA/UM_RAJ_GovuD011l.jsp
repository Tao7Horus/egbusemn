...
//    String ApprovalCode=uagb030.getApprovalCode(uac.getID());
            ...
            //if (upcheNm != "") {
            totalCount = ctl.approvalList_count(upcheNm, Start, End, "S", total, ApprovalCode, area, instituCd);
            if (totalCount > 0) {
                ettlist = ctl.approvalList(Integer.parseInt(page_no), PAGE_SIZE, upcheNm, Start, End, "S", total, ApprovalCode, area, instituCd);
            }
            //}
%>
...
<html> 
    <head>
        ...
        <SCRIPT LANGUAGE="JavaScript">
            ...
            function toView(agr_g2bCode,agr_recept,agr_LicenseCode,agr_ApprovalCode,groupNo, departmentNo){
    ...
    <body>
        ...
        <div class="col-750 clearfix last">
            <h1 class="pageTitle"><i class="icon-title"></i>Tra cứu đăng kí bên mời thầu</h1>
            ...
            <form name="useForm">
                <table width="100%" border="0" cellspacing="1" cellpadding="2" class="tr">
                    ...
                    <tr>
                        ...
                        <td colspan="3" class="tdb"><input type="text" name="instituCd"	value="<%=instituCd%>" class="read" size="20" >	</td>
                    </tr>
                    <tr height=30>
                        <td style="width: 20%" class="tdar"><!-- 수요기관명 --> Tên bên mời thầu</td>
                        <td colspan="3" class="tdb"><input type="text" name="upcheNm"	value="<%=upcheNm%>" class="read" style="width: 100%">	</td>
                        
                    </tr>
                    <tr height=30>
                        <td class="tdar" style="width: 20%"><!-- 상태구분 -->Phân loại trạng thái</td>
                        <td class="tdb" colspan="3" style="width: 80%">
                            ...
                            <input type="radio"		name="total"	value="3" onfocus="blur()" <%=(total.equals("3")) ? "checked " : ""%>><!-- 취소 --> Bảo lưu
                            <input type="radio"		name="total"	value="4" onfocus="blur()" <%=(total.equals("4")) ? "checked " : ""%>><!-- 취소 --> Từ chối
                            <input type="radio"		name="total"	value="5" onfocus="blur()" <%=(total.equals("5")) ? "checked " : ""%>><!-- 보류 --> Hủy bỏ
                            <% } else {%>
                            ...
                            <input type="radio"		name="total"	value="3" onfocus="blur()" ><!-- 취소 -->Bảo lưu
                            <input type="radio"		name="total"	value="4" onfocus="blur()" ><!-- 취소 -->Từ chối
                            <input type="radio"		name="total"	value="5" onfocus="blur()" ><!-- 보류 -->Hủy bỏ
                            <% }%>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="width: 15%" class="tdar"><!-- 신청기간 -->Thời gian đăng ký </td>
                        <td class="tdb" style="width: 35%">
           ...
           <%if (totalCount > 0) { %>
                <table width="100%" border="0" cellspacing="1" cellpadding="2" class="tr">
                    <tr>
                        <td height="4" colspan="9" class="line"></td>
                    </tr>
                    <tr height="30">
                        <td	width="3%"		class="tdac"><!-- 번호 -->STT</td>
                        <td	width="12%"		class="tdac"><!-- 신청일자 -->Ngày đăng ký</td>
                        <td	width="15%"		class="tdac"><!-- 수요기관명 -->Tên cơ quan bên mời thầu</td>
                        <td	width="8%"		class="tdac"><!-- 수요기관명 -->Mã cơ quan</td>
                        <td	width="8%"		class="tdac"><!-- 사업자등록번호 -->Số ĐKKD</td>
                        <td	width="15%"		class="tdac"><!-- 담당자명 -->Tên người phụ trách</td>
                        <td	width="14%"		class="tdac"><!-- 주&nbsp;&nbsp;&nbsp;소 -->Địa chỉ</td>
                        <td	width="12%"		class="tdac"><!-- 여부 -->Trạng thái</td>
                        <td	width="13%"		class="tdac"><!-- 여부 -->Tỉnh / Thành phố</td>
                    </tr>
                    <%	
                                    for (int i = 0; i < ettlist.length; i++) {
                    %>
                    <tr>
                        <td class="tdbc"><%=((Integer.parseInt(page_no) - 1) * PAGE_SIZE) + i + 1%></td>
                        <td class="tdbc"><%=ettlist[i].getrecept()%></td>
                        <td class="tdb">
                            <a href="javascript:toView('<%=ettlist[i].getg2bCode()%>','<%=ettlist[i].getdependCode()%>','<%=ettlist[i].getLicenseCode()%>','<%=ettlist[i].getLicenseCode()%>','<%=ettlist[i].getGroupNo()%>','<%=ettlist[i].getDepartmentNo()%>');">
                    ...
                    <tr>
                        <td height="2" colspan="9" class="line"></td>
                    </tr>
                </table>
                ...
                <table>
                	<tr>
                        <td style="text-align: center;" colspan="8"><!-- 조회결과가 없습니다.[검색조건 입력후 조회 바랍니다.] -->Không có kết quả tìm kiếm.[Nhập điều kiện tìm kiếm để tìm kiếm.]</td>
                    </tr>
                </table>
                <%	}%>

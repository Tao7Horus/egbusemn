...
public class EP_COV_GTQ953 extends HttpServlet
{
    ...
    public void service(final HttpServletRequest httpServletRequest, final HttpServletResponse httpServletResponse) throws ServletException, IOException {
        ...
        ResultSet set = null;
        Trx trx = null;
        final String s = " select count(*) from UM_PUB_INSTITU_MAST where INSTITU_CD like ?||'%' and INSTITU_FULL_NM like '%'||?||'%'";
        final String s2 = " select t, INSTITU_CD, INSTITU_FULL_NM, DELETE_YN from( \tselect INSTITU_CD, INSTITU_FULL_NM, DELETE_YN, rownum t \tfrom ( \t\tselect INSTITU_CD, INSTITU_FULL_NM, DELETE_YN  \t\tfrom UM_PUB_INSTITU_MAST \t\twhere INSTITU_CD like ?||'%' \t\tand INSTITU_FULL_NM like '%'||?||'%' \t\torder by 2 \t) ) where t > ? and t < ? ";
        try {
            ...
            if (b) {
                ...
                preparedStatement.setString(1, retSpace.trim());
                preparedStatement.setString(2, replace.trim());
    ...
    private void initHtml(final PrintWriter printWriter, final String s, final String s2, final String s3, final String s4, final String s5, final String s6) {
        ...
        printWriter.println("<title>Tìm kiếm cơ quan</title>");
        ...
        printWriter.println("\t\t\talert('Hãy nhập ít nhất 2 ký tự cho mã cơ quan hoặc cho tên công ty.');");
        ...
        printWriter.println(" \t\t\t\talert(\"Hãy bỏ ký tự đặc biệt ra khỏi mã cơ quan đã nhập.[!@#$%^&*()_-+|/;:,.?`~{}[]]\");");
        printWriter.println("\t\t\t\treturn;");
        printWriter.println(" \t\t\t}");
        printWriter.println(" \t\t}");
        printWriter.println(" \t\tfor(var i=0;i< va2.length;i++){");
        printWriter.println(" \t\t\tif(sp_char.indexOf(va2.charAt(i)) != -1){");
        printWriter.println(" \t\t\t\talert(\"Hãy bỏ ký tự đặc biệt ra khỏi tên công ty đã nhập.[!@#$%^&*()_-+|/;:,.?`~{}[]]\");");
        printWriter.println("\t\t\t\treturn;");
        printWriter.println(" \t\t\t}");
        printWriter.println(" \t\t}");
        printWriter.println("\t\tdocument.ebid.method='post';");
        printWriter.println("\t\tdocument.ebid.action\t='/servlet/dao/EP_COV_GTQ953';\t");
        ...
        printWriter.println("               <td width=* style='padding-top:2.0pt' class='HEADLINE'  background='/img/pop_tit2.gif'   style='background-position:right; background-repeat:no-repeat;'>&nbsp;Tìm kiếm cơ quan</td>    ");
        ...
        printWriter.println("<tr>");
        printWriter.println("<td class='tda1'>Tên cơ quan</td>");
        ...
        printWriter.println("</tr>");
        printWriter.println("<tr>");
        printWriter.println("\t<td\tclass='tda1'>Mã cơ quan</td>");
    ...
    private void noDataHtml(final PrintWriter printWriter) {
        ...
        printWriter.println("<td class=tdac>Mã cơ quan</td>");
        printWriter.println("<td class=tdac>Tên cơ quan</td>");
        printWriter.println("</tr>");
        printWriter.println("<tr>");

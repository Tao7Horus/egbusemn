...
public class UM_RAB_MastGood
{
    public CommEntity[] getList(final String bizRegNo, final Connection conn) throws Exception {
        final Vector parameterV = new Vector();
        try {
            final String sql = "select CD_ITEM, MAST_YN_ITEM, UPDATE_DT from UM_SUPPLIER_ITEMS where BIZ_REG_NO = ?";
    ...
    public void delete(final String bizRegNo, final Connection conn) throws Exception {
        ...
        try {
            ...
            sql.append(" DELETE FROM UM_SUPPLIER_ITEMS ");
    ...
    public void save(final String bizRezNo, final List goodList, final Connection conn) throws Exception {
        ...
        try {
            ...
            GoodItem item = null;
            final int totalGoodList = goodList.size();
            if (totalGoodList > 0) {
                for (int i = 0; i < totalGoodList; ++i) {
                    item = (GoodItem) goodList.get(i);
                    sql.append(" into UM_SUPPLIER_ITEMS (BIZ_REG_NO, CD_ITEM, MAST_YN_ITEM, UPDATE_DT)");

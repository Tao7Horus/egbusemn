...
public class UM_ADB_ConrA010p_imsi
{
    public UM_RAE_ConuB010b[] select_comlist(final int n, final int n2, String s, String s2, String s3, String replace, final String s4, final String s5, final String s6, final String s7) {
        ...
        s2 = ((s2 == null) ? "" : s2);
        s3 = ((s3 == null) ? "" : s3);
        Log.debug("UM_ADB_ConrA010p_imsi.java's id = " + s7);
        String s8 = " SELECT 사업자등록번호, 신청_변경구분, 신청일자, 상호명, 대표자명, 우편번호, 주소,   \t\t\t\t\t        나머지주소, 전화번호, 홈페이지, 진행상태, 처리사유, XML문서위치, 전송번호, ROWNUM N  FROM \t\t        (SELECT 사업자등록번호, 신청_변경구분, 신청일자, 상호명, 대표자명, 우편번호, 주소,                              나머지주소, 전화번호, 홈페이지, 진행상태, 처리사유, XML문서위치, 전송번호, ROWNUM N                FROM (SELECT 사업자등록번호, 신청_변경구분, 신청일자, 상호명, 대표자명, 우편번호, 주소,           \t\t            나머지주소, 전화번호, 홈페이지, 진행상태, 처리사유, XML문서위치, 전송번호       \t               FROM 사용_전자문서상태                         WHERE 신청_변경구분 = '1'      \t                AND 진행상태 IN('1')      \t";
        if (0 < replace.length()) {
    ...
    public int Max_count(String s, String s2, String s3, String replace, final String s4, final String s5, final String s6, final String s7) {
        ...
        s3 = ((s3 == null) ? "" : s3);
        try {
            String s8 = "SELECT count(*) FROM   \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t( SELECT 사업자등록번호, 신청_변경구분, 신청일자, 상호명, 대표자명, 우편번호, 주소,       \t\t         나머지주소, 전화번호, 홈페이지, 진행상태, 처리사유, XML문서위치, 전송번호          \t    FROM (SELECT 사업자등록번호, 신청_변경구분, 신청일자, 상호명, 대표자명, 우편번호, 주소, \t     \t\t     나머지주소, 전화번호, 홈페이지, 진행상태, 처리사유, XML문서위치, 전송번호  \t\t        FROM 사용_전자문서상태  \t\t\t\t\t\t           WHERE 신청_변경구분 = '1'     \t\t\t\t\t \t         AND 진행상태 IN('1')      \t\t\t\t\t\t";
            if (0 < replace.length()) {
                s8 = s8 + " AND 사업자등록번호 like '%" + replace + "%'";
            }
            if (0 < s4.length()) {
            ...
            Log.debug("UM_ADB_ConrA010p_imsi.Max_count block SQLException : " + ex.toString());
        }
        catch (Exception ex2) {
            Log.debug("UM_ADB_ConrA010p_imsi.Max_count block Exception : " + ex2.toString());
        }
        finally {
            if (executeQuery != null) {
                try {

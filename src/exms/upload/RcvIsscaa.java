...
public class RcvIsscaa
{
    ...
    public RcvIsscaa(final Connection conn) {
        this.pconn = null;
        this.n = 0;
        this.parser = new DOMParser();
        this.xmlDoc = new XMLDocument();
        this.serialNumber = 0L;
        this.sourceName = "RcvIsscaa";
        this.pconn = conn;
    ...
    public String main_process(final String xmlfile, final String fullpath) {
        ...
        try {
            ...
            if (e.getErrorCode() == 1) {
                return "-2[RcvIsscaa - 부정당업체등록통보서]oracle error 0001";
            }
            return String.valueOf(e.getErrorCode()) + "[RcvIsscaa - 부정당업체등록통보서]oracle error " + e.getErrorCode() + ":" + e.toString();
        }
        catch (Exception e2) {
            if (this.pconn != null) {
                try {
            ...
            return "-2[RcvIsscaa - 부정당업체등록통보서]DB Update Error" + e2.getMessage();
        }
        finally {
            if (fin != null) {
                try {
    ...
    private void updateMaster1(final Connection pconn) throws Exception {
        ...
        try {
            query = " UPDATE  UM_VIOLATE_COM SET                     \n          DOC_NO = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Header.Details/cc:Document.Number.Text/Text.Content", 0) + "',      \n" + "         RELEASE_CLS = 'Y',                         \n" + "         RELEASE_DT = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.Cancellation.Date/DateTime.Content", 0) + "',      \n" + "         RELEASE_RSON = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.CancellationReason.Text/Text.Content", 0) + "',        \n" + "         RELEASE_UM_ID = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Header.Details/cc:Message.Sender.Identifier/Identifier.Content", 0) + "'  \n" + " WHERE    \t\t\t\t\t\t\t  \n" + "         BIZ_REG_NO = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Organization.Information/Organization.Details/cc:Organization.Identifier/Identifier.Content", 0) + "'  \n" + "   AND   PUNISH_COUNT       = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.Number.Text/Text.Content", 0) + "'";
            pstmt = pconn.prepareStatement(query);
            pstmt.executeUpdate();
            this.SaveLog("UM_VIOLATE_COM 수정 : BIZ_REG_NO[" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Organization.Information/Organization.Details/cc:Organization.Identifier/Identifier.Content", 0) + "], 제재횟수[" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.Number.Text/Text.Content", 0) + "]");
        }
        catch (Exception exm) {
            this.SaveLog(" updateMaster1() query[" + query + "] :" + exm.toString());
            throw exm;
        }
        finally {
            if (pstmt != null) {
    ...
    private void updateMaster2(final Connection pconn) throws Exception {
        final String cnt2 = "";
        final String date = "";
        final String endDate = this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Details/Expiration.Date/DateTime.Content", 0);
        final String SaupNo = this.GetValue(this.xmlDoc, "/gb:Isscaa/Organization.Information/Organization.Details/cc:Organization.Identifier/Identifier.Content", 0);
        final String count = this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.Number.Text/Text.Content", 0);
        final String CancelDate = this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.Cancellation.Date/DateTime.Content", 0);
        ...
        try {
            ...
            query = " SELECT TO_CHAR(MIN(a.PUNISH_DT), 'YYYYMMDD'), TO_CHAR(MAX(a.EXPIRE_DT), 'YYYYMMDD')  FROM UM_VIOLATE_COM a, UM_SUPPLIER_ENTER_MAST b  WHERE a.BIZ_REG_NO = '" + SaupNo + "' " + "   AND a.INSTITU_CD NOT IN ('" + this.GetValue(this.xmlDoc, "/gb:Isscaa/PublicOrganization.Details/Organization.Details/cc:Organization.Identifier/Identifier.Content", 0) + "') " + "   AND a.BIZ_REG_NO = b.BIZ_REG_NO " + "   AND (a.SUSPEND_CLS = 'N' OR (a.SUSPEND_CLS = 'Y' AND a.RESUMPTION_CLS = 'Y')) " + "   AND a.RELEASE_CLS = 'N' " + "   AND TO_CHAR(a.EXPIRE_DT, 'yyyy-mm-dd') >= TO_CHAR(SYSDATE, 'yyyy-mm-dd') " + "   AND TO_CHAR(a.PUNISH_DT, 'yyyy-mm-dd') <= TO_CHAR(SYSDATE, 'yyyy-mm-dd') ";
            pstmt = pconn.prepareStatement(query);
            rs = pstmt.executeQuery();
            if (rs.next()) {
            ...
            if (Integer.parseInt(CancelDate) <= Integer.parseInt(nowDate)) {
                if (count2.equals("0")) {
                    ...
                    pstmt = pconn.prepareStatement(query);
                    pstmt.executeUpdate();
                    this.SaveLog("UM_ENTER_STATE 해제 삭제 : BIZ_REG_NO[" + SaupNo + "]");
                }
                else if (!sDate.equals("") && !eDate.equals("")) {
                    query = " UPDATE UM_ENTER_STATE SET \n RAISED_DT = SYSDATE, \n START_DT = '" + sDate + "', \n" + " END_DT = '" + eDate + "', \n" + " REMARK     = '" + Gita + "', \n" + " MANAGER_ID = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Header.Details/cc:Message.Sender.Identifier/Identifier.Content", 0) + "', \n" + " PROCESS_DT = sysdate \n" + " WHERE BIZ_REG_NO = '" + SaupNo + "' \n" + "   AND STATE_CLS = '05'";
                    if (pstmt != null) {
                        try {
                            pstmt.close();
                        }
                    ...
                    pstmt = pconn.prepareStatement(query);
                    pstmt.executeUpdate();
                    this.SaveLog("UM_ENTER_STATE 해제 수정 : BIZ_REG_NO[" + SaupNo + "]");
                }
            }
        }
        catch (Exception exe) {
            this.SaveLog(" updateMaster2() :" + exe.toString());
            throw exe;
        }
        finally {
            if (rs != null) {
    ...
    private void updateDetail(final Connection pconn) throws Exception {
        final String endDate = this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Details/Expiration.Date/DateTime.Content", 0);
        final String unpairCount = this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.Number.Text/Text.Content", 0);
        final String SaupNo = this.GetValue(this.xmlDoc, "/gb:Isscaa/Organization.Information/Organization.Details/cc:Organization.Identifier/Identifier.Content", 0);
        final String CancelDate = this.GetValue(this.xmlDoc, "/gb:Isscaa/Sanction.Cancellation.Details/Sanction.Cancellation.Date/DateTime.Content", 0);
        final String Gita = String.valueOf(SaupNo) + "/" + unpairCount;
        final XMLElement xemt = (XMLElement)this.xmlDoc.getDocumentElement();
        final NodeList pNode = this.xmlDoc.selectNodes("/gb:Isscaa/Organization.Information/CeoList/CeoItem/cc:Organization.CEO.Identifier/Identifier.Content", (NSResolver)xemt);
        String nowDate = "";
        String sDate = "";
        String eDate = "";
        String count4 = "";
        ...
        try {
            ...
            for (int i = 0; i < pNode.getLength(); ++i) {
                sql2 = " SELECT BIZ_REG_NO FROM UM_REPR \n WHERE REPR_IDENT_NO = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Organization.Information/CeoList/CeoItem/cc:Organization.CEO.Identifier/Identifier.Content", i) + "' \n" + "   AND BIZ_REG_NO NOT IN ('" + SaupNo + "') ";
                if (rs != null) {
                    try {
                        rs.close();
                    }
                ...
                while (rs.next()) {
                    ...
                    if (!sangtaeCount.equals("0") && Integer.parseInt(CancelDate) <= Integer.parseInt(nowDate)) {
                        if (count4.equals("0")) {
                            ...
                            this.SaveLog("UM_ENTER_STATE 해제 삭제 : BIZ_REG_NO[" + UnpairNo + "]");
                        }
                        else {
                            ...
                            pstmt3 = pconn.prepareStatement(query);
                            pstmt3.executeUpdate();
                            this.SaveLog("UM_ENTER_STATE 해제 수정 : BIZ_REG_NO[" + UnpairNo + "]");
                        }
                    }
                }
            }
    ...
    private void updateDetailBRCEO(final Connection pconn) throws Exception {
        ...
        try {
            ...
            String UnpairNo = "";
            for (int i = 0; i < pNode.getLength(); ++i) {
                sql2 = "\t\tSELECT BIZ_REG_NO\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tFROM  UM_SUPPLIER_ENTER_MAST\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tWHERE CORP_REG_NO IN (  SELECT CORP_REG_NO\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t  FROM UM_SUPPLIER_ENTER_MAST x, UM_REPR y        \t\t\t\t\t\t\t\t\t\t\t  WHERE x.BIZ_REG_NO = y.BIZ_REG_NO\t\t\t\t\t\t\t\t\t\t\t\t\t  AND  y.REPR_IDENT_NO = '" + this.GetValue(this.xmlDoc, "/gb:Isscaa/Organization.Information/CeoList/CeoItem/cc:Organization.CEO.Identifier/Identifier.Content", i) + "' \n" + "\t\t\t\t\t\t\t\t\t\t\t)\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t" + "       AND BIZ_REG_NO NOT IN ('" + SaupNo + "')\t\t\t\t\t\t\t\t\t\t\t\t";
                if (rs != null) {
                    try {
                        rs.close();
                    }
    ...
    private void SaveLog(final String msg) {
        Log.info("xmlFileName:" + this.xmlFileName + " ISSCAA" + this.sourceName + "[" + this.serialNumber + "]:" + msg);
    }
}

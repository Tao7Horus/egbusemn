...
public class UM_ADV_GovrR010 extends HttpServlet
{
    ...
    private void headHtml(final PrintWriter out, final String desformat, final String province) throws Exception {
        out.println("<html>");
        out.println("<head>");
        out.println("<title>지청별기관등록현황</title>");
        ...
        out.println("<tr>");
        out.println("  <td bgcolor='#EDEDED' height='40' style='border:.5pt\tsolid windowtext;verticla-align:center;' colspan=7>");
        out.println("    <p align='center'><font size='5' face='굴림체'><b>지청별 조회</b></font></p></td>");
        out.println("</tr>");
        out.println("</table>");
        out.println("<br>");
        out.println("<table x:str border=0 cellpadding=0 cellspacing=0 style='border-collapse: collapse;' width=95% align=center>");
        out.println("  <tr>");
        out.println("    <td colspan=7 align=right><font size=2><b>관할지청</b>:" + new Common_ComBo().getCodeName("J03", province) + "</font></td>");
        out.println("  <tr>");
        out.println("    <td colspan=7 align=right><font size=2><b>날짜</b>:" + new CommServer().getCurrDate(0, 0) + "</font></td>");
        out.println("  </tr>");
        out.println("</table>");
        out.println("<br>");
    }
    ...
    private void displayResult(final PrintWriter out, final ResultSet rest) throws Exception {
        ...
        while (rest.next()) {
            ...
            out.println("    <td class=retdbc>" + rest.getString(6) + "</td>");
            out.println("    <td class=retdb>" + rest.getString(7) + "</td>");
            out.println("</tr>");
            ++i;
        }
        if (i == 1) {
            out.println("  <tr>");
            out.println("  <td colspan=7 class=retdbc>조회된 결과가 없습니다.</td>");
            out.println("  </tr>");
        }
        out.println("</table>");
        out.println("<br>");

...
public class UM_ADV_GovrB022c extends HttpServlet
{
    public void doPost(final HttpServletRequest req, final HttpServletResponse res) throws ServletException, IOException {
        ...
        while (true) {
            try {
                ...
                gongReceive.updateReceiveGonggonMaster(restoration, recept, "E", userID, conn);
                if (LicenseGubun.equals("Y")) {
                    ...
                    gongCertReceive.updateReceiveGonggonCert(restoration, LicenseCode, "E", userID, conn);
                }
                gongCertReceive.updateReceiveGonggonCert("Bảo lưu", recept, "E", userID, conn);
                conn.commit();
                conn.setAutoCommit(true);
                CommonMessage.printMsg(null, "7", "Đã bảo lưu thông tin bên mời thầu thành công!", null, res);
            }
            catch (Exception ex) {
                try {
                    if (conn != null) {
                ...
                Log.debug("UM_ADV_GovrB022c.java Số tiếp nhận[" + recept + "], Mã bên mời thầu[" + LicenseCode + "],Số tiếp nhận chứng nhận số[" + LicenseGubun + "], UserID[" + userID + "]:" + ex.toString());

...
public class UM_ADV_GovrA010c_pdnt_multi extends HttpServlet {
...
//	    	}  
    		...
    		if(sso.getID() == null || !"a".equals(userCls)) {
        	...
        	if(selectedIndex == null || (selectedIndex != null && selectedIndex.length == 0)) {
        		throw new Exception("Chưa có nhà thầu nào được chọn để phê duyệt !");
        	}
        	lstNhaThau = new CoQuanNhaThau[selectedIndex.length];
        	for (int i = 0; i < selectedIndex.length; i++) {
				...
				Log.debug("Nhà thầu: index = " + index + "; g2BCode = " + g2BCode + "; COMNO = " + comNo +"; recept = " + recept + "; dependCode = " + dependCode + ";  ");
				CoQuanNhaThau nhaThau = new CoQuanNhaThau(index, g2BCode, recept, dependCode, testOptionYN, comNo);
				lstNhaThau[i] = nhaThau;
			}
            ...
            for (int i = 0; i < lstNhaThau.length; i++) {
            	...
            	try {
            		...
            		///validate begin
	            	if("".equals(lstNhaThau[i].getG2BCode())) {
	            		lstNhaThau[i].setHavingErrors(true);
	            		lstNhaThau[i].setErrorString("Lỗi truyền dữ liệu: Không tìm thấy mã cơ quan.");
	            		continue;
	            	}
	            	
	            	if("".equals(lstNhaThau[i].getRecept())) {
	            		lstNhaThau[i].setHavingErrors(true);
	            		lstNhaThau[i].setErrorString("Lỗi truyền dữ liệu: Không tìm thấy mã tiếp nhận đăng ký.");
	            		continue;
	            	}
	            	
	            	if("".equals(lstNhaThau[i].getDependCode())) {
	            		lstNhaThau[i].setHavingErrors(true);
	            		lstNhaThau[i].setErrorString("Lỗi truyền dữ liệu: Không tìm thấy mã phê duyệt đăng ký CTS.");
	            		continue;
	            	}
	            	if("".equals(lstNhaThau[i].getComNo())) {
	            		lstNhaThau[i].setHavingErrors(true);
	            	...
	            	//Kiểm tra nhà thầu đã được đăng ký chưa
	            	if (urc010.isDataExistOnEnterMaster(lstNhaThau[i].getComNo(), con) == 0) { 
	            		lstNhaThau[i].setHavingErrors(true);
	            		lstNhaThau[i].setErrorString("Không thể thực hiện phê duyệt chứng nhận số cho các Doanh nghiệp chưa đăng kí<br> Xác nhận đăng kí cơ quan trong menu Đăng kí cơ quan/ Tra cứu<br> sau đó,. nếu bạn thực hiện phê duyệt thì đồng thời đăng kí chứng nhận số cũng được thực hiện phê duyệt.");
                        continue;
		        	}
	            	
	            	String regStatus = gongReceive.getRegCertStatus(lstNhaThau[i].getRecept(), con);
	            	if (!regStatus.equals("N") && !regStatus.equals("E")) { 
	            		lstNhaThau[i].setHavingErrors(true);
	            		lstNhaThau[i].setErrorString("Nhà thầu này đã được chấp nhận hoặc xử lý.");
	            		continue;
	            	}
	            	
	            	//khởi tạo các object tạo CTS
	            	Log.debug("$$$$$$$$$$$$$$$$$ UM_ADV_GovrA010c_pdnt Thuc hien sinh Ma phe duyet Chung thu so trong Buoc 3 recvNo = " + lstNhaThau[i].getRecept());
                    Secu secu2 = null;
            		...
            		UM_ADJ_GovuA020b ettcode  = ctl.Approval_Confirm2(lstNhaThau[i].getG2BCode(), lstNhaThau[i].getDependCode(), lstNhaThau[i].getRecept());
            		...
            		//Khởi tạo CTS mới
            		res = iCK.regNewUser(strNAME, USRID, COMNO, EMAIL, strTELNO, strHP_NO, strFAXNO, "00", "ADDRESS", "COMPN", "hawk");
            		if (res == null) {
            		...
            		if (!refername.equals("") || !refercode.equals("")) {
                        ...
                        urc010.updateReception("Y", "", lstNhaThau[i].getRecept(), lstNhaThau[i].getG2BCode(), lstNhaThau[i].getDependCode(), uac.getID(), con);
                        
                        //Cập nhật nội dung CTS vừa tạo vào bảng UM_REC_PUB_INSTITU_CERT
                        urc010.updateReceptionCert(dn, refername, refercode, COMNO, lstNhaThau[i].getG2BCode(), lstNhaThau[i].getDependCode(), con);
                        lstNhaThau[i].setReferCode(refercode);
                        lstNhaThau[i].setReferName(refername);
                        Log.debug("Đăng ký CTS thành công cho nhà thầu có số ĐKKD: " + lstNhaThau[i].getG2BCode() + "; Mã tiếp nhận CTS: " + lstNhaThau[i].getRecept() + "; Tên CTS: " + dn);
		            } else {
		            	throw new RuntimeException("Phê duyệt chứng thư số không thành công, không khởi tạo được chứng thư số mới.");
		            }
            		...
            		try {
                    	...
                    	Log.debug(this.getClass().getName() + " Số ĐKKD ["+ lstNhaThau[i].getG2BCode() + "], Số tiếp nhận [" + lstNhaThau[i].getRecept() + "], Mã phê duyệt đăng ký [" + lstNhaThau[i].getDependCode() +"] :" + ex3.toString());
                        lstNhaThau[i].setHavingErrors(true);
                    ...
                    try {
                    	...
                    	Log.debug(this.getClass().getName() + " Số ĐKKD ["+ lstNhaThau[i].getG2BCode() + "], Số tiếp nhận [" + lstNhaThau[i].getRecept() + "], Mã phê duyệt đăng ký [" + lstNhaThau[i].getDependCode() +"] :" + ex4.toString());
                        lstNhaThau[i].setHavingErrors(true);
                		lstNhaThau[i].setErrorString(ex4.toString());
                        continue;
                    }
                    Log.debug(this.getClass().getName() + " Số ĐKKD ["+ lstNhaThau[i].getG2BCode() + "], Số tiếp nhận [" + lstNhaThau[i].getRecept() + "], Mã phê duyệt đăng ký [" + lstNhaThau[i].getDependCode() +"] :" + e.toString());
                    lstNhaThau[i].setHavingErrors(true);
            ...
            for (int j = 0; j < lstNhaThau.length; j++) {
				if(lstNhaThau[j].havingErrors) {
					notificationContent = notificationContent + "- Nhà thầu số " + lstNhaThau[j].getIndex() + " Số ĐKKD " + lstNhaThau[j].getG2BCode() + ": Thất bại => " + lstNhaThau[j].getErrorString() + "<br/>";
				} else {
					notificationContent = notificationContent + "- Nhà thầu số " + lstNhaThau[j].getIndex() + " Số ĐKKD " + lstNhaThau[j].getG2BCode() + ": Thành công ! Mã nhận CTS: " + lstNhaThau[j].getReferCode()  + " - " + lstNhaThau[j].getReferName() + "<br/>";
					successCount++;
				}
			}
	...
	private class CoQuanNhaThau {
		...
		protected CoQuanNhaThau() {
			super();
		}

		protected CoQuanNhaThau(int index, String g2bCode, String recept, String dependCode, String testOptionYN, String comNo) {
			super();
			this.index = index;

...
public class DB2TXT06_rex extends HttpServlet
{
    public void service(final HttpServletRequest req, final HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            fileWrite = String.valueOf(fileWrite) + "<라이센스><![CDATA[" + licenseName + "]]></라이센스>";
            if (rs != null) {
                fileWrite = String.valueOf(fileWrite) + "<sub1>";
                while (rs.next()) {
                    fileWrite = String.valueOf(fileWrite) + "<row><지역><![CDATA[" + ComStr.checkNull(rs.getString(1)) + "]]></지역>" + "<시공능력평가액><![CDATA[" + ComStr.checkNull(rs.getString(2)) + "]]></시공능력평가액>" + "<상호><![CDATA[" + ComStr.checkNull(rs.getString(3)) + "]]></상호>" + "<사업자등록번호><![CDATA[" + ComStr.checkNull(rs.getString(4)) + "]]></사업자등록번호>" + "<대표자><![CDATA[" + ComStr.checkNull(rs.getString(5)) + "]]></대표자>" + "<주민번호><![CDATA[" + ComStr.checkNull(rs.getString(6)) + "]]></주민번호>" + "<주소><![CDATA[" + ComStr.checkNull(rs.getString(7)) + "]]></주소>" + "<전화번호><![CDATA[" + ComStr.checkNull(rs.getString(8)) + "]]></전화번호>" + "<팩스번호><![CDATA[" + ComStr.checkNull(rs.getString(9)) + "]]></팩스번호>" + "<기업구분><![CDATA[" + ComStr.checkNull(rs.getString(10)) + "]]></기업구분>" + "</row>";
                }
                fileWrite = String.valueOf(fileWrite) + "</sub1>";
            }
            rs.close();

...
public class UM_GOV_ConiD025c
{
    ...
    public UM_GOV_ConiD025c() {
        this.dxeHost = "XLNDB";
        this.dxePort = 1050;
        this.dxeStore = "usemn";
        this.XlnSourceMaster = String.valueOf(this.dxeStore) + ":/xml/Isscaa000g.xml";
        this.dxeUser = "";
        this.dxePassword = "";
    ...
    public String loadDB2XML(final String mCode, final String id, final String unpairNo, final String punishmentCount) {
        String result = "";
        final UM_GOV_ConiD021c um_gov_conid021c = new UM_GOV_ConiD021c();
        try {
            ...
            if (this.rs.next()) {
                ...
                if (this.rs1 != null && this.rs1.next()) {
                    ...
                    um_gov_conid021c.문서번호[0] = docNo;
                    um_gov_conid021c.문서명[0] = "부정당업자 제재해제통보서";
                    um_gov_conid021c.입력자ID[0] = id;
                    um_gov_conid021c.공공기관명[0] = KigwanNm;
                    um_gov_conid021c.담당자성명[0] = chargeNm;
                    um_gov_conid021c.담당자전화번호[0] = chargeTel;
                    um_gov_conid021c.공공기관코드[0] = mCode;
                    um_gov_conid021c.공공기관명1[0] = "조달청 G2B 시스템";
                    um_gov_conid021c.담당자성명1[0] = "";
                    um_gov_conid021c.사업자등록번호[0] = saup_no;
                    um_gov_conid021c.법인등록번호[0] = lawNo;
                    um_gov_conid021c.상호명[0] = upcheNm;
                    um_gov_conid021c.우편번호[0] = bonsapostNo;
                    um_gov_conid021c.주소1[0] = bonsaAddr;
                    um_gov_conid021c.주소2[0] = bonsaAddrNa;
                    um_gov_conid021c.영업종목[0] = businItem;
                    um_gov_conid021c.면허번호[0] = licenseNo;
                    um_gov_conid021c.제재근거_1[0] = punishmentBasic;
                    um_gov_conid021c.제재근거_2[0] = punishmentBasic2;
                    um_gov_conid021c.계약종류[0] = notifyKind;
                    um_gov_conid021c.제재횟수[0] = Count;
                    um_gov_conid021c.제재년월일[0] = punishmentStart;
                    um_gov_conid021c.만료년월일[0] = punishmentEnd;
                    um_gov_conid021c.제재기간[0] = punishmentPeriod;
                    um_gov_conid021c.해약년월일[0] = cancelDay;
                    um_gov_conid021c.비고[0] = gita;
                    PutValues(dMaster, um_gov_conid021c.문서번호);
                    PutValues(dMaster, um_gov_conid021c.문서명);
                    PutValues(dMaster, um_gov_conid021c.작성일자);
                    PutValues(dMaster, um_gov_conid021c.입력자ID);
                    PutValues(dMaster, um_gov_conid021c.공공기관명);
                    PutValues(dMaster, um_gov_conid021c.담당자성명);
                    PutValues(dMaster, um_gov_conid021c.담당자전화번호);
                    PutValues(dMaster, um_gov_conid021c.공공기관코드);
                    PutValues(dMaster, um_gov_conid021c.공공기관명1);
                    PutValues(dMaster, um_gov_conid021c.담당자성명1);
                    PutValues(dMaster, um_gov_conid021c.사업자등록번호);
                    PutValues(dMaster, um_gov_conid021c.법인등록번호);
                    PutValues(dMaster, um_gov_conid021c.상호명);
                    PutValues(dMaster, um_gov_conid021c.우편번호);
                    PutValues(dMaster, um_gov_conid021c.주소1);
                    PutValues(dMaster, um_gov_conid021c.주소2);
                    PutValues(dMaster, um_gov_conid021c.영업종목);
                    PutValues(dMaster, um_gov_conid021c.면허번호);
                    PutValues(dMaster, um_gov_conid021c.제재근거_1);
                    PutValues(dMaster, um_gov_conid021c.제재근거_2);
                    PutValues(dMaster, um_gov_conid021c.계약종류);
                    PutValues(dMaster, um_gov_conid021c.제재횟수);
                    PutValues(dMaster, um_gov_conid021c.제재년월일);
                    PutValues(dMaster, um_gov_conid021c.만료년월일);
                    PutValues(dMaster, um_gov_conid021c.제재기간);
                    PutValues(dMaster, um_gov_conid021c.해약년월일);
                    PutValues(dMaster, um_gov_conid021c.비고);
                }
            }
            if (this.psmt != null) {
                try {
            ...
            if (this.rs2 != null) {
                ...
                while (this.rs2.next()) {
                    ...
                    um_gov_conid021c.일련번호1[0] = Integer.toString(j + 1);
                    um_gov_conid021c.대표자명[0] = isStrNull(this.rs2.getString(1));
                    um_gov_conid021c.대표자주민번호[0] = isStrNull(JuminNo);
                    um_gov_conid021c.대표자주소[0] = isStrNull(this.rs2.getString(3));
                    dpPutValues(dMaster, um_gov_conid021c.일련번호1, this.일련번호1, j, 1);
                    dpPutValues(dMaster, um_gov_conid021c.대표자명, this.대표자명, j, 0);
                    dpPutValues(dMaster, um_gov_conid021c.대표자주민번호, this.대표자주민번호, j, 0);
                    dpPutValues(dMaster, um_gov_conid021c.대표자주소, this.대표자주소, j, 0);
                    ++j;
                }
            }
            this.rs.close();
    ...
    public static void dpPutValues(final XMLDocument doc, final String[] node, final String[] nodeNameType, final int index, final int first) {
        try {
            ...
            if (first == 1) {
                ...
                final Node ageNode = doc.selectNodes("/gb:Isscaa/Organization.Information/CeoList/CeoItem", (NSResolver)xemt).item(index);
                ageNode.getParentNode().insertBefore(addEle, ageNode);
            }
            addEle = doc.createElement(nodeNameType[0]);
            tmpNodeList = doc.selectNodes("/gb:Isscaa/Organization.Information/CeoList/CeoItem", (NSResolver)xemt);
